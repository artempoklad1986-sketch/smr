...
if (!isset($moduleDB['invoice_pro'])) {
    ...
    dbSaveModuleDB('invoice_pro', ['settings' => $initSettings, 'index' => []]);
    $moduleDB['invoice_pro'] = ['index' => [], 'settings' => $initSettings];
}
...
// Читаем настройки из модульной БД (dbGetModuleDB — правильное место хранения)
$_invModuleData = $moduleDB['__own__'] ?? dbGetModuleDB('invoice_pro');
$settings = $_invModuleData['settings'] ?? ($moduleDB['invoice_pro']['settings'] ?? []);
$index    = $_invModuleData['index']    ?? ($moduleDB['invoice_pro']['index']    ?? []);
...
function invRmIndex(array &$db, $id) {
    $ownData = dbGetModuleDB('invoice_pro');
    $index   = $ownData['index'] ?? [];
    $index   = array_values(array_filter($index, fn($i) => $i['id'] !== $id));
    dbSaveModuleDB('invoice_pro', ['index' => $index]);
}
...
function invSaveHtml($id, $html) {
  ...
  return $proto . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost') . '/public/invoices/' . $name;
}
...
case 'list': {
 ...
 case 'get': {
    ...
    dbSaveModuleDB('invoice_pro', ['settings' => $settings]);
}
...
case 'save_settings': {
    ...
    // Сохраняем настройки
   // Сохраняем настройки через правильный механизм модульной БД
dbSaveModuleDB('invoice_pro', ['settings' => $settings]);

// Читаем обратно для подтверждения
$savedData = dbGetModuleDB('invoice_pro');
$savedSettings = $savedData['settings'] ?? $settings;
...
const uploadUrl = (typeof API_URL !== 'undefined' ? API_URL : '/api/api.php')
    + '?module=invoice_pro&action=upload_image&key='
